...
The bisection bandwidth of a network is the bandwidth that can be attained when dividing the network in to two equally large parts (w.r.t. the number of servers). The bisection bandwidth most often is used to describe the worst case, i.e. the bisection with the lowest bandwidth.
...
3
---
The oversubscription rate at each level is 1:1. This means that all pairs of nodes can fully utilize their links simultaneously, giving a full bisection bandwidth.
...
============
- Network OS
- Programmable interface on the switches